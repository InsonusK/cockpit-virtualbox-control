...
Каждый PR в `master` проверяется workflow `.github/workflows/pr-master.yml`. Проверки (типы, тесты, версия) запускаются только если PR меняет что-то в `src/` или `tests/`; если изменений там нет, обе джобы пропускаются. Когда изменения есть: прогоняются типы (`npm run typecheck`) и тесты (`npm test`), а также проверяется, что `version` в `src/manifest.json` (формат `X.Y.Z`) увеличен относительно версии в `master` — без этого PR замёржить нельзя.

После мержа PR в `master` workflow `.github/workflows/release-master.yml` запускается, только если PR менял что-то в `src/` или `tests/` (иначе, например для чисто документационного PR, релиз не создаётся). Если изменения были, он:

1. собирает `npm run build`;
2. упаковывает `dist/` в `dist.tar.gz`;
3. считает его `sha256` и публикует рядом как `dist.tar.gz.sha256`;
4. публикует оба файла как asset'ы GitHub Release с тегом `v<версия из src/manifest.json>`, автоматически сгенерированными release notes (список смёрженных PR со времени прошлого релиза, `--generate-notes`) вдобавок к заголовку с номером PR.

Скомпилированный JS в git не попадает вообще — ни в `master`, ни куда-либо ещё.

Деплой на сервер с Cockpit делает ansible: скачивает `dist.tar.gz` с нужного GitHub Release этого репозитория, проверяет его по `dist.tar.gz.sha256` (единственная проверка целостности на этом пути — файл иначе тянется по обычному URL без какого-либо ещё пиннинга) и распаковывает. Node.js/npm на целевых хостах не нужен — единственная сборка происходит в CI.

По умолчанию плагин разворачивается per-user, в `~/.local/share/cockpit/virtualbox` — виден только той ОС-учётке, в чьём home он лежит. Если плагин должен быть виден сразу всем пользователям хоста, есть альтернатива — системный путь `/usr/share/cockpit/virtualbox` (нужен root; директория должна оставаться root-owned и не быть group/world-writable, иначе теряется весь смысл системного пути). Ни один из вариантов не меняет, с какими привилегиями выполняется `VBoxManage` — плагин никогда не запрашивает superuser-мост Cockpit, поэтому команды всегда идут от имени того ОС-пользователя, что открыл сессию. Подробные шаги для обоих вариантов — в `docs/skills/devops/virtualbox-cockpit-plugin-install.skill/procedure-prod-deploy.md`.
